  ...
  Object.keys(_fields).forEach(n => {
    ...
    if (Array.isArray(fc.args)) {
      ...
      Object.keys(fc.args).forEach(argName => {
        args[argName] = _objectSpread({}, (0, _is.isObject)(fc.args[argName]) ? fc.args[argName] : null, {
...
function convertObjectTypeArrayAsThunk(types, sc) {
  ...
  return t.map(type => {
    ...
    if (!tc && isThunk) {
      return new _ThunkComposer.ThunkComposer(() => sc.typeMapper.convertOutputTypeDefinition(type), (0, _typeHelpers.getComposeTypeName)(type));
    }
...
function convertInterfaceArrayAsThunk(types, sc) {
  ...
  return t.map(type => {
    ...
    return isThunk ? new _ThunkComposer.ThunkComposer(() => sc.typeMapper.convertInterfaceTypeDefinition(type), (0, _typeHelpers.getComposeTypeName)(type)) : sc.typeMapper.convertInterfaceTypeDefinition(type);
  });
}